...
//import Prando from "prando";  //Imported in main
//import { fy_shuffle, int16_as_bytes } from "./utils"; //added to this class
...
function fy_shuffle(array, rand) {
...
class SFX {
  ...
  to_cluck_or_not_to_cluck(origSfx,cluck,flap) {
    //Randomly decide if the sfx will be chicken
    let chicken = null;
    if(sfx.target_set == cluck.sfx_set)
      chicken = cluck;
    else if(sfx.target_set == flap.sfx_set)
      chicken = flap;
    else
      return origSfx;
    items = [origSfx,chicken];

    return items[Math.floor(Math.random()*items.length)];
  }

  randomize_sfx(romData,cluckerizer=false) {
    const sfx_table = {
      2: 0x1A8BD0,
      3: 0x1A8CCC,
    };
    ...
    let cluck = SFX.create_sfx("Cucco", 0x02, 0x30, 0x1ef1, []);
    let flap  = SFX.create_sfx( "Flapping (Duck/Cucco swarm/Ganon bats/Keese/Raven/Vulture)", 0x03, 0x1e, 0x2091, []);
    ...
    sfx_sets.forEach((shuffled_sfx) => {
      for (const id in shuffled_sfx) {
        ...
        let base_address = this.snes_to_pc(sfx_table[sfx.target_set]);
        /*
        rom.write(
        ...
        */
        if(cluckerizer) {
          let chicken = to_cluck_or_not_to_cluck(sfx,cluck,flap);
          sfx.addr = chicken.addr;
          sfx.target_chain = chicken.target_chain;    
        }
        romData = this.seekWrite(romData,base_address + sfx.target_id * 2 - 2,int16_as_bytes(sfx.addr))
        let ac_base = this.snes_to_pc(sfx_accompaniment_table[sfx.target_set]);
        ...
        if (sfx.target_chain) {
          sfx.target_chain.forEach((chained) => {
            ...
            romData = this.seekWrite(romData,ac_base + last - 1, (cluckerizer == true) ? to_cluck_or_not_to_cluck(sfx,cluck,flap).addr : chained);
            last = chained;
          });
        }
        // 0 indicates the end of the chain (unchained ones always have zero here)
        //rom.write(ac_base + last - 1, 0);
        romData = this.seekWrite(romData,ac_base + last - 1, 0);
      }
    });
    return romData;
  }

  seekWrite(rom, seek, bytes) {
